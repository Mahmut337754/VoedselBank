            ...
            <div class="col p-4">
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0"><?= htmlspecialchars($klant['gezinsnaam']) ?></h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <h6><i class="bi bi-envelope"></i> Contactgegevens</h6>
                                <table class="table table-sm table-borderless">
                                    <tr>
                                        <th style="width:120px;">Telefoon:</th>
                            ...
                            <div class="col-md-6">
                                <h6><i class="bi bi-people"></i> Gezinssamenstelling</h6>
                                <table class="table table-sm table-borderless">
                                    <tr>
                                        <th style="width:120px;">Volwassenen:</th>
                        ...
                        <hr>
                        <h6><i class="bi bi-shield-exclamation"></i> Dieetwensen / Allergieën</h6>
                        <?php if (empty($gekoppeldeWensen)): ?>
                            <p class="text-muted">Geen geregistreerde wensen.</p>
                        <?php else: ?>
                            <div class="d-flex flex-wrap gap-2">
                                <?php foreach ($gekoppeldeWensen as $wens): ?>
                                    <span class="badge bg-info text-dark p-2"><?= htmlspecialchars($wens['naam']) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="card-footer bg-transparent">
                        <a href="<?= BASE_URL ?>/klanten/edit?id=<?= $klant['klant_id'] ?>" class="btn btn-warning"><i class="bi bi-pencil"></i> Bewerken</a>
                        <a href="<?= BASE_URL ?>/klanten/delete?id=<?= $klant['klant_id'] ?>" class="btn btn-danger" onclick="return confirm('Weet u zeker dat u deze klant wilt verwijderen?')"><i class="bi bi-trash"></i> Verwijderen</a>
                    </div>
                </div>
            </div>
        </div>
